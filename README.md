...
- Signal sniffing can be done via a Logic Analyzer. (will be used when I am going to switch from the existing WiFi module to the ESP32 CAM module, to validate what the previous WiFi chip used to send to the RF MCU). Another way to understand the signal commands is to use a USB to TTL adapter as mentioned [here](https://github.com/arjunsk/reversing-wifi-uav)
